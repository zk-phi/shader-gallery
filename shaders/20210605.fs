...
float dist(vec3 pos) {
  return min4(
    sphere(rep3(pos, 8.0), 1.0),
    tube(rep2(pos.xz, 8.0), .5),
    tube(rep2(pos.yx, 8.0), .5),
    tube(rep2(pos.yz, 8.0), .5)
  );
}
...
void main( void ) {
  ...
  for (int i = 0; i < 35; i++) {
    ...
    if (dist < EPS) {
      ...
      float depth = pow(clamp(distance(campos, march) / 30.0, 0.0, 1.0), 2.0);
      color = mix(color, vec3(light) + vec3(.2, .2, .2), 1.0 - depth);
      break;
    }
